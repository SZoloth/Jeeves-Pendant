...
const writeCursor = (obj) =>
  fs.writeFile(CURSOR_FILE, JSON.stringify(obj, null, 0));
...
const apiURL =
  ...
  `start=${encodeURIComponent(cursor.lastEnd)}&limit=1&direction=asc&includeMarkdown=true`;

const pendant = await mustJson(
  await fetch(apiURL, { headers: { "X-API-Key": process.env.LIMITLESS_API_KEY }}),
  "Limitless"
);
...
/* 3️⃣  write block under today's page */
const payload = {
  action: "create-block",
